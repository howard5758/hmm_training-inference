    ...
    def readFile(self, numseq, flag):
        ...
        for i in range(0, numseq):
            ...
            for j in range(0, 201):
                temp = f.readline()
                allStateSeq[i].append(self.xyToInt[int(temp[0])-1][int(temp[2])-1])
                allObsSeq[i].append(self.obsToInt[temp[4]])
            f.readline()

        # save to different parameters depending on flag
        if flag == "train":
            self.trainState = allStateSeq
            self.trainOutput = allObsSeq
        if flag == "test":
            self.testState = allStateSeq
            self.testOutput = allObsSeq
